...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        if len(state.stack) is 0:
            input += [None, None, None]
        elif len(state.stack) is 1:
            input += [state.stack[-1], None, None]
        elif len(state.stack) is 2:
            input += [state.stack[-1], state.stack[-2], None]
        else:
            input += [state.stack[-1], state.stack[-2], state.stack[-3]]

        if len(state.buffer) is 0:
            input += [None, None, None]
        elif len(state.buffer) is 1:
            input += [state.buffer[-1], None, None]
        elif len(state.buffer) is 2:
            input += [state.buffer[-1], state.buffer[-2], None]
        else:
            input += [state.buffer[-1], state.buffer[-2], state.buffer[-3]]

        for i in range(len(input)):
            if input[i] is None:
                input[i] = self.word_vocab['<NULL>']
            elif pos[input[i]] is 'CD':
                input[i] = self.word_vocab['<CD>']
            elif pos[input[i]] is 'NNP':
                input[i] = self.word_vocab['<NNP>']
            elif input[i] is 0:
                input[i] = self.word_vocab['<ROOT>']
            elif words[input[i]].lower() not in self.word_vocab:
                input[i] = self.word_vocab['<UNK>']
            else:
                input[i] = self.word_vocab[words[input[i]].lower()]
    ...
    def get_output_representation(self, output_pair):  
        ...
        output = keras.utils.to_categorical(self.output_labels[output_pair], num_classes=91)
